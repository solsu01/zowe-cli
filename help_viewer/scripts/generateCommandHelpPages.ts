...
import { DefaultHelpGenerator, ICommandDefinition, Imperative, ImperativeConfig, IO } from "@zowe/imperative";
...
function writeTreeData(config: ImperativeConfig) {
...
(async () => {
    ...
    const metadataFile = path.join(cmdDocsDir, "..", "metadata.json");
    let cachedMetadata: IPackageMetadata[] = [];
    if (fs.existsSync(metadataFile)) {
        cachedMetadata = JSON.parse(fs.readFileSync(metadataFile).toString());
    }
    const currentMetadata: IPackageMetadata[] = calcPackageMetadata(
        myConfig.callerPackageJson,
